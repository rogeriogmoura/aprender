<?php
require_once __DIR__ . '/_conexao_mysqli.php';

class CriarTabelaOSItem {
  public function up($mysqli) {
    $sql = "
      CREATE TABLE IF NOT EXISTS os_item (
        id INT AUTO_INCREMENT PRIMARY KEY,
        os_id INT NOT NULL,
        tipo ENUM('peca','servico') NOT NULL,
        descricao VARCHAR(255) NOT NULL,
        quantidade DECIMAL(10,2) NOT NULL DEFAULT 1,
        valor_unitario DECIMAL(10,2) NOT NULL DEFAULT 0.00,
        criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_item_os FOREIGN KEY (os_id) REFERENCES os(id) ON DELETE CASCADE
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
    ";
    if (!$mysqli->query($sql)) throw new Exception($mysqli->error);
  }

  public function down($mysqli) {
    if (!$mysqli->query("DROP TABLE IF EXISTS os_item")) throw new Exception($mysqli->error);
  }
}

try {
  $mysqli = conexao_mysqli();
  $m = new CriarTabelaOSItem();
  if (isset($_GET['acao']) && $_GET['acao'] === 'down') {
    $m->down($mysqli);
    echo "DOWN: tabela os_item removida.";
  } else {
    $m->up($mysqli);
    echo "UP: tabela os_item criada.";
  }
} catch (Exception $e) {
  echo "Erro: " . $e->getMessage();
}
